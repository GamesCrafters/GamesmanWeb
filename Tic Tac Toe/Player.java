public class Player {
    ...
    public int[] chooseMove(int alpha, int beta, int depth, boolean ally) {
        int[] out = new int[3], moves[] = board.possibleMoves(), reply = new int[3];
        int best = 0;
        for (int i = 0; i < moves.length; i++) {
            ...
            if (depth > 1) {
                ...
                reply = foe.chooseMove(alpha, beta, depth - 1, !ally);
            } else {
                reply[0] = x;
                reply[1] = y;
                reply[2] = board.evaluateBoard(color);
            }
            board.remove(x,y); 
    ...
    public void makeMove() {
        ...
        board.add(x,y, color);
        System.out.print("Machine player makes move:(");
        System.out.print(x);
        System.out.print(", ");
        System.out.print(y);
        System.out.println(").");
