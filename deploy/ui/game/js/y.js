...
var drawLine = function(start, end, segments, lastRow) {
  ...
  for(var i = 0; i <= segments; i++ ) {
	...
	centers[id] = new Point(setX, setY, BOARD_STARTING_COLOR.red,  BOARD_STARTING_COLOR.green, BOARD_STARTING_COLOR.blue, 1);
	if(lastRow) {
		centers[id].fringe = true;
	}
	if((i==0) || (i==segments)) {
...
var drawEdges = function(circleSet) {
	...
	for(var c in circleSet) {
		for(var i in circleSet[c].edges) {
			context.moveTo(circleSet[c].edges[i].c1.x, circleSet[c].edges[i].c1.y);
			context.lineTo(circleSet[c].edges[i].c2.x, circleSet[c].edges[i].c2.y);
...
var mag = function(v1, v2) { return Math.sqrt(Math.pow(v1.x-v2.x,2)+Math.pow(v1.y-v2.y,2));}
